...
_HAS_QWEN3 = False
try:
    # Import Qwen3V model and processor
...
def extract_multimodal_embeddings(text: str, image_path: str, device: str = None, model_name: str = None) -> Dict[str, torch.Tensor]:
    ...
    if _HAS_QWEN3:
        # Use Qwen3V when available. Allow overriding model_name.
        ...
        logger.debug("Loading processor from path: %s", model_path)
        processor = processor_cls.from_pretrained(
            model_path,
            local_files_only=True,
            trust_remote_code=True
        )
        logger.info("Processor loaded from %s", model_path)
    ...
    raise RuntimeError("Qwen3V model not available. Install a transformers version that provides Qwen3V support.")
